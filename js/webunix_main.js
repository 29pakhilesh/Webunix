// js/webunix_main.js - Core OS, Window Manager & Start Menu Fixed
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderStartMenuContent(menuEl) {
        ...
        let profileIconHTML;
        if (avatarUrl) {
            profileIconHTML = `<img src="${avatarUrl}" style="width:36px; height:36px; border-radius:50%; object-fit:cover; border:2px solid var(--accent-color);">`;
        } else {
            const color = "hsl(" + (username.length * 50) + ", 70%, 50%)";
            profileIconHTML = `<div class="sm-avatar-circle" style="width:36px; height:36px; border-radius:50%; background:${color}; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:16px;">${username[0].toUpperCase()}</div>`;
        }
        ...
        menuEl.innerHTML = `
            <div class="sm-grid-container">
                ${apps.map(app => `<div class="sm-app-card" onclick="${app.action}"><div class="sm-icon-square" style="background:${app.bg}">${app.icon}</div><span class="sm-app-label">${app.name}</span></div>`).join('')}
            </div>
            <div class="sm-system-bar">
                ...
                <button class="sm-power-btn" onclick="window.openShutdownMenu()"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18.36 6.64a9 9 0 1 1-12.73 0"></path><line x1="12" y1="2" x2="12" y2="12"></line></svg></button>
            </div>
        `;
    }
    ...
    startBtn.onclick = (e) => {
        ...
        const MENU_WIDTH = 500; // Must match CSS width
        const BUTTON_CENTER_X = startBtnRect.left + (startBtnRect.width / 2);
        
        // Set left position to center the menu above the button
        startMenu.style.left = (BUTTON_CENTER_X - (MENU_WIDTH / 2)) + "px";
        
        // Set bottom position 5px above the taskbar's top edge (eliminates overlap)
        startMenu.style.bottom = (window.innerHeight - taskbarRect.top + 15) + "px"; // Increased margin to 15px
