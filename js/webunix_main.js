...
// --- Global helper to bring a process window to the front ---
    window.bringToFrontByPID = (pid) => {
    // Finds the window element using PID and increases its z-index
    const proc = window.kernel?.process.list().find(p => p.pid === parseInt(pid));
    if (proc && proc.window && window.wm) {
        proc.window.style.zIndex = ++window.wm.zIndex;
    }
    };
    ...
    // Removed window.bringToFrontByPID helper function as it's no longer needed
    ...
    function renderStartMenuContent(menuEl) {
        ...
        // --- STATIC APPLICATIONS LIST (Original Content) ---
        // Removed the "Active Windows" section
        html += `<div class="menu-header">Applications</div>`;
        html += `
            <div class="menu-item" onclick="launch('Terminal')">董 Terminal</div>
            <div class="menu-item" onclick="launch('FileManager')">刀 File Manager</div>
            <div class="menu-item" onclick="launch('Editor')">統 Text Editor</div>
            <div class="menu-item" onclick="launch('Settings')">笞呻ｸSettings</div>
        `;
        
        if(installed.includes("calculator")) html += `<div class="menu-item" onclick="launch('Calculator')">ｧｮ Calculator</div>`;
        
        // Add a dedicated entry to open the Process Manager
        html += `<div class="menu-sep"></div>
                 <div class="menu-item" onclick="window.openProcessManager()">Task Manager</div>`;


        // --- SYSTEM ACTIONS (Original Content) ---
        html += `<div class="menu-sep"></div>
                 <div class="menu-item danger" onclick="window.openShutdownMenu()">閥 Shut Down</div>`;
...
// ... (rest of main.js remains the same)
    ...
    startBtn?.addEventListener("click", () => {
        ...
        // Always render content before toggling visibility
    ...
    window.launch = (app) => {
        if(startMenu) startMenu.classList.add("hidden");
        // In a real system, we'd check if running and bring to front, but here we just launch
        if(app === "Terminal") window.openTerminal();
        if(app === "FileManager") window.openFileManager();
        if(app === "Editor") window.openEditor();
        if(app === "Settings") window.openSettings();
